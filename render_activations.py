...
def render_set(n, channel):
    ...
    t_rgb = t_image[...,:3]
    t_alpha = t_image[...,3:]
    t_bg = image_sample([1, w, h, 3], sd=0.2, decay_power=1.5)
    print(t_bg, t_alpha, objectives.as_objective(obj)(T))
    exit()
    ...
    t_crop = tf.random_crop(t_crop, [1, 110, 110, 4])
    t_crop_rgb, t_crop_alpha = t_crop[..., :3], t_crop[..., 3:]
    t_crop_rgb = linear2gamma(t_crop_rgb)
    model.import_graph(t_crop_rgb, scope="alpha")
    ...
    #tf.losses.add_loss(-objective_f(T)*(1.0-t_alpha_mean_full)*0.5)
    #tf.losses.add_loss(-objective_f(T)*(1.0-t_alpha_mean_crop))

    obj2 = objectives.as_objective(obj)(T)
    tf.losses.add_loss(-obj2*(1.0-t_alpha_mean_full)*0.5)
    tf.losses.add_loss(-obj2*(1.0-t_alpha_mean_crop))
    ...
    if not os.path.exists(f_model) or is_force:
...
is_force = dargs["--force"]
    
for channel in CHANNELS:
    for n in COLORSET:

        f_image = os.path.join(save_image_dest, channel + f"_{n}.jpg")
        if os.path.exists(f_image) and not is_force:
            continue
